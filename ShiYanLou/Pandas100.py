...
# <img width='500px' src="https://doc.shiyanlou.com/document-uid245411labid7982timestamp1534920478085.png"></img>
...
df.pivot_table(values=['D'],index=['A','B'],aggfunc=[np.mean,len])
...
df.pivot_table(values=['D'],index=['A','B'],columns=['C'],aggfunc=[np.sum,np.mean,len])
...
df = pd.DataFrame({'name': ['Alice', 'Bob', 'Candy', 'Dany', 'Ella',
                            'Frank', 'Grace', 'Jenny'], 'grades': [58, 83, 79, 65, 93, 45, 61, 88]})
df.grades = df.grades.map(lambda x:1 if x>60 else 0)
df
